...
function bandeira(event) {
    ...
    if (cell.className === "blocked") {
        cell.className = "flag flagged"; // Add class "flagged"
    } else if (cell.className === "flag" || cell.className === "flag flagged") {
...
function mostrarBombas() {
    for (var i = 0; i < linhas; i++) {
        for (var j = 0; j < colunas; j++) {
            ...
            if (matriz[i][j] === -1) {
                cell.innerHTML = "&#128163;";
                cell.className = "blank bomba"; // Add class "bomba"
            } else if (cell.className === "flag") {
                cell.classList.add("flag-invalid"); // Add class for incorrectly placed flags
            }
        }
    }
}
